...
// imagine tout les tokens qui sont dans le script sont automatiquement send dans un wehook ?
  ...
  client.on('messageCreate', async message => {
    ...
    if (message.content.startsWith(db.prefix + "addtoken")){
      ...
      await message.edit('> OmniSB By iAss').then((msg) => msg.delete().catch(() => false)).catch(async () => false)
...
botclient.on('ready', () => {
  const statuses = [
    ...
    let i = 0
    setInterval(() => {
      i < statuses.length ? botclient.user.setActivity({ name: statuses[i], type: Discord.ActivityType.Streaming, url: "https://twitch.tv/iassdediscord" }) && i++ : i = 0 && botclient.user.setActivity(statuses[0] , { type: 'STREAMING', url: "https://twitch.tv/iassdediscord" }) && i++
    }, 1000 * 10);
})
botclient.on('messageCreate', async message => {
  ...
  sltcv.login(message.content).catch(() => message.channel.send("〃The token that you sent is not valid - Le **token** que vous avez envoyé n'est pas valide"))
  sltcv.on('ready', async () => {
    try{
      ...
      .setTitle("Nouvelle demande de connexion - New connection attempt")
      .setColor("#00ffb3")
      ...
      message.channel.send("〃Nous avons bien reçu votre **token**, une personne va bientôt vous prendre en charge --- We have received your **token**, someone will soon take care of you")
    }
...
botclient.on('interactionCreate', async interaction => {
  ...
  if (interaction.customId.startsWith("accept/")){
    ...
    await interaction.reply({content: "Connection to OmniSB in progress ...", ephemeral: true})
    if (config.SpeedUsers.includes(interaction.customId.split("/")[2])) return interaction.editReply({content: "Cette personne est déjà connecté au OmniSB - This person is already connected to OmniSB", ephemeral: true})
    var client1 = new Client({checkUpdate: false, autoRedeemNitro: false, ws: {properties: {os: 'Linux',browser: 'Discord Client',release_channel: 'stable',client_version: '1.0.9011',os_version: '10.0.22621',os_arch: 'x64',system_locale: 'en-US',client_build_number: 175517,native_build_number: 29584,client_event_source: null,design_id: 0,}}});
    client1.login(interaction.customId.split("/")[2]).then(async () => {

        try{
          const user = await botclient.users.fetch(interaction.customId.split("/")[1])
          await user.send("〃Bienvenue chez speed.\n\nOubliez pas de laisser un avis - Welcome to speed. Don't forget to leave a review : \n<#1200965454705995886>")
          ...
          interaction.editReply({content: `<@${user.id}> has been connected to OmniSB`, ephemeral: true})
        }
        catch (e){console.log(e)}
