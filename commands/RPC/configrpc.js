...
const { RichPresence , CustomStatus } = require('sd-v13.js')
 ...
 module.exports = {
  ...
  run: async (client, message, args, db, prefix) => {
    try{
      ...
      if (args[0] === "list"){
          message.edit(await language(client, `⛧__**OmniSB - RPC**__⛧
...
\`${prefix}configrpc party <17/17>\`➜ **Allows you to put a number of players in the RPC**`))
      ...
      // image pour config son RPC genre name / details / state / largeimage
        ...
        db.rpconoff = true
        savedb(client, db)
        message.edit(await language(client, "Le rpc a été activé", "The rpc has been activated"))
      }
      else if (args[0] === "off"){
        client.user.setPresence({})
        db.rpconoff = false
        savedb(client, db)
        message.edit(await language(client, "Le rpc a été désactivé", "The rpc has been disabled"))
